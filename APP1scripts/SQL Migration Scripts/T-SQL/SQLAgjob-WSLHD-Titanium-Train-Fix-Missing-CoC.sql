...
/****** Object:  Job [Titanium_WS_train fix missing CoC]    Script Date: 18/10/2021 11:22:55 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 18/10/2021 11:22:55 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Titanium_WS_train fix missing CoC', 
		@enabled=1, 
		...
		@notify_email_operator_name=N'eHealthDBA', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Run Script]    Script Date: 18/10/2021 11:22:56 ******/
...
where CAST(linternal AS BIGINT) & CAST(0x400 AS BIGINT) > 0) --appointment

select a.uidCOT into #apptswithoutcoc from appts a where not exists (select null from CoCs c where a.uidcot = c.uidcot)
order by a.uidcot
...
rollback tran
		...
		@database_name=N'Titanium_WS_train', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Weekday hourly', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=62, 
		@freq_subday_type=8, 
		...
		@active_start_date=20190626, 
		@active_end_date=99991231, 
		@active_start_time=70000, 
		@active_end_time=180000, 
		@schedule_uid=N'0c783310-4709-4b23-ac3c-fbb5424e057e'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
