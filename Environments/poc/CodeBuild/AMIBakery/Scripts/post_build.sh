...
test -f manifest.json || (aws events put-events --entries file://Environments/poc/CodeBuild/AMIBakery/Scripts/ami_event_fail.json --region $1 ; exit 1)
...
aws ssm put-parameter --cli-input-json '{"Type": "String", "Name": "/poc/web/amiid/latest", "Value": "'"$AMI_ID"'", "Overwrite": true}' --region $1

# Send events
sed -i.bak "s/<<AMI-ID>>/$AMI_ID/g" Environments/poc/CodeBuild/AMIBakery/Scripts/ami_event_succeed.json
aws events put-events --entries file://Environments/poc/CodeBuild/AMIBakery/Scripts/ami_event_succeed.json --region $1
